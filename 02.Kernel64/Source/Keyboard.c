...
//check if there is received data in output buffer(0x60)
BOOL kIsOutputBufferFull()
{
  //if status register(0x64) is 1, data exists
...
BOOL kActivateKeyboard()
{
  int i;
  int j;
  //activate keyboard:: 0xAE is activation code on control register
  ...
  //activate keyboard:: 0xF4 is activation code on input buffer
  kOutPortByte(0x60,0xF4);

  //wait until ACK arrive 
  for(j=0;j<100;j++)
  {
    for(i=0;i<0xFFFF;i++)
    {
...
void kEnableA20Gate()
{
    ...
    //wait until input buffer(0x60)is empty
    for( i = 0 ; i < 0xFFFF ; i++ )
    {
        if( kIsOutputBufferFull() == TRUE )
            break;
...
//check if scan code is num/symbol
...
//check if scan code is from num pad
...
//check if scan code uses combined key
...
//check combined key and sync led status
